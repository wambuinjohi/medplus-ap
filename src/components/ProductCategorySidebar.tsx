...
export default function ProductCategorySidebar({
  ...
  // Use provided categories or fall back to dynamic categories (preserve DB slug)
  const categories = providedCategories || dynamicCategories.map((cat) => ({ name: cat.name, slug: (cat as any).slug }));
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
          ...
          <nav className="divide-y divide-gray-100 md:max-h-96 md:overflow-y-auto">
            {categories.map((category) => {
              ...
              const slug = (category as any).slug || getCategorySlug(category.name);
              return (
                <Link
                  key={category.name}
                  to={`/products/${slug}`}
