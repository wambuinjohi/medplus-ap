...
export default function CustomerStatementPreviewModal({
  ...
  // Get outstanding invoices (net of credit note allocations per invoice)
  ...
  const outstandingInvoices = customerInvoices.filter(inv => getNetOutstanding(inv) > 0.005);
  ...
  const handleGeneratePDF = async () => {
    try {
      ...
      const companyDetails = companies?.[0] ? {
        ...
        website: (companies?.[0] as any)?.website
      } : undefined;
                ...
                <Table>
                  ...
                  <TableBody>
                    {outstandingInvoices.map((invoice) => {
                      const outstanding = getNetOutstanding(invoice);
