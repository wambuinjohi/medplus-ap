...
export const exportCustomerStatementDetailToExcel = (
  ...
  payments: any[],
  filename?: string,
  options?: ExcelExportOptions & { creditNoteAllocations?: any[] }
) => {
  const today = new Date();
  const { title, companyInfo, creditNoteAllocations = [] } = (options as any) || {};

  // Build map of credit allocated per invoice (net outstanding)
  const creditByInvoice = new Map<string, number>();
  (creditNoteAllocations || []).forEach((a: any) => {
    if (!a.invoice_id) return;
    creditByInvoice.set(a.invoice_id, (creditByInvoice.get(a.invoice_id) || 0) + (Number(a.allocated_amount) || 0));
  });
  ...
  // Get outstanding invoices with aging classification (net of credit notes)
  const statementDate = today.toISOString().split('T')[0];
  const outstandingInvoices = invoices
    .filter(inv => getNetOutstanding(inv) > 0.005)
    .map(inv => {
      const outstanding = getNetOutstanding(inv);
