...
export default function ProductDetail() {
  ...
  // Debug: log category/variant resolution for mobility-aids issue
  useEffect(() => {
    if (!categoryLoading && !variantLoading) {
      console.log('[ProductDetail] slug:', productSlug, 'category:', category, 'variants:', variants?.length, 'variant:', variant, 'isCategory:', !!category, 'isVariant:', !!variant && !isCategory);
    }
  }, [productSlug, category, variants, variant, categoryLoading, variantLoading, isCategory, isVariant]);
  ...
  if (!variant && !isCategory) {
    console.warn('[ProductDetail] NOT FOUND redirect', { productSlug, category, variant, categoryLoading, variantLoading });
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center">
        <h1 className="text-2xl font-bold mb-2">Product not found: {productSlug}</h1>
        <p className="text-gray-600 mb-4">Category: {String(!!category)} Variant: {String(!!variant)} Loading: {String(categoryLoading||variantLoading)}</p>
        <Link to="/products" className="text-primary underline">Back to Products →</Link>
        <p className="text-xs text-gray-400 mt-4">Check console for [ProductDetail] debug. Try hard refresh Ctrl+Shift+R.</p>
      </div>
    );
  }
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="flex-1">
                ...
                {variants.length === 0 ? (
                  <div className="text-center py-12 bg-white rounded-lg border">
                    <p className="text-gray-600 mb-2">No products in {category?.name} yet.</p>
              ...
              {variants.map((v) => (
                ...
                >
                  {/* Variant Images - clickable to specific product */}
                  <Link to={`/products/${v.slug}`} className="block relative h-48 bg-gray-200 overflow-hidden cursor-pointer">
                  ...
                  <div className="p-4">
                    ...
                    {/* Variant Name - clickable to specific product */}
                    <Link to={`/products/${v.slug}`} className="block hover:text-primary transition-colors">
                      <h3 className="text-base font-bold text-gray-900 mb-4 line-clamp-2 hover:text-primary">
                        {v.name}
                      </h3>
                    </Link>
                    ...
                    <div className="flex gap-2">
                      <Link to={`/products/${v.slug}`} className="flex-1">
                        <Button variant="outline" className="w-full text-sm">View Details</Button>
                      </Link>
                      <button
                        onClick={() => handleQuotationRequestClick(v)}
                        className="flex-1 bg-gradient-to-r from-blue-500 to-green-500 text-white font-bold py-2 px-4 rounded-lg hover:shadow-lg transition-all duration-300 hover:scale-105 text-sm flex items-center justify-center gap-2"
                      >
