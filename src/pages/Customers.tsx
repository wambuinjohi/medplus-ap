...
export default function Customers() {
  ...
  const filteredCustomers = customers?.filter(customer => {
    ...
    const matchesCreditLimit = creditLimitFilter === 'all' ||
      ...
      (creditLimitFilter === 'override_enabled' && customer.allow_credit_beyond_limit) ||
      (creditLimitFilter === 'limit_enforced' && customer.credit_limit && !customer.allow_credit_beyond_limit);
  ...
  const handleViewStatement = async (customer: Customer) => {
    try {
      ...
      const [invoicesResponse, paymentsResponse, creditNoteAllocations] = await Promise.all([
        supabase
          .from('invoices')
          .select('id, invoice_date, invoice_number, total_amount, due_date, status, paid_amount')
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select value={creditLimitFilter} onValueChange={setCreditLimitFilter}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="override_enabled">Override Enabled</SelectItem>
                        <SelectItem value="limit_enforced">Limit Enforced</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
