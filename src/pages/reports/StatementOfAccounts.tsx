...
const computeCustomerStatements = (customers: any[], invoices: any[], payments: any[], creditNoteAllocations: any[] = []) => {
  ...
  return customers.map(customer => {
    ...
    // Calculate aging analysis (net of credit note allocations per invoice)
    const today = new Date();
    let current = 0, days30 = 0, days60 = 0, days90 = 0;

    const creditByInvoice = new Map<string, number>();
    customerCreditNoteAllocations.forEach((a: any) => {
      const iid = a.invoice_id;
      if (!iid) return;
      creditByInvoice.set(iid, (creditByInvoice.get(iid) || 0) + (Number(a.allocated_amount) || 0));
    });

    const getNetOutstanding = (invoice: any) => {
      const gross = (Number(invoice.total_amount) || 0) - (Number(invoice.paid_amount) || 0);
      const credited = creditByInvoice.get(invoice.id) || 0;
      return Math.max(0, gross - credited);
    };

    const outstandingInvoices = customerInvoices.filter(invoice => getNetOutstanding(invoice) > 0.005);

    outstandingInvoices.forEach(invoice => {
      const dueDate = new Date(invoice.due_date || invoice.invoice_date);
      const daysPastDue = Math.floor((today.getTime() - dueDate.getTime()) / (1000 * 60 * 60 * 24));
      const unpaidAmount = getNetOutstanding(invoice);
