...
export const useConvertQuotationToInvoice = () => {
  ...
  return useMutation({
    mutationFn: async (quotationId: string) => {
      ...
      // Credit limit check before conversion
      {
        ...
        if (customer && customer.credit_limit > 0) {
          ...
          const newOutstanding = totalOutstanding + quotation.total_amount;

          if (newOutstanding > customer.credit_limit && !customer.allow_credit_beyond_limit) {
            throw new Error('Quotation conversion would exceed customer credit limit. Admin must enable credit override for this account.');
          }
        }
      }
...
export const useUpdateInvoiceWithItems = () => {
  ...
  return useMutation({
    mutationFn: async ({ invoiceId, invoice, items }: { invoiceId: string; invoice: any; items: InvoiceItem[] }) => {
      ...
      const { data, error } = await supabase.rpc('update_invoice_with_items_atomic', {
